...
use sqlx::{Database, Encode, FromRow, QueryBuilder, Row, Type, query_builder::Separated, sqlite::SqliteRow};

use super::{
    ENTRIES_TABLE, ENTRIES_TAG_TABLE, TAGS_TABLE, Db, DbError,
    Id, ReadingTime, Result, Timestamp,
};
use crate::storage::repository::{CreateTag, Detail, TagRow};
...
) -> Result<Vec<(EntryRow, Vec<TagRow>)>> {
...
pub async fn create(
    ...
    entry: CreateEntry,
    tags: &[CreateTag],
    tag_repo: &dyn crate::storage::repository::TagRepository,
    ...
    if !tags.is_empty() {
        tag_repo.create_and_link_tags(id, tags).await.map_err(|e| DbError::RepositoryError(e.to_string()))?;
    }
...
mod tests {
    ...
    use sqlx::SqlitePool;
    use std::sync::Arc;
    use crate::storage::repository::{SqliteTagRepository, TagRepository};
